...
class SupportClass extends BaseClass
{
 ...
 WHERE p.weight=255 AND m.weight=255";
 ...
 function get_modules_list_header() {
  $ary = array();
  $sql = "SELECT m.mid, m.name, m.url, m.url_flag, m.weight, m.submenu
 FROM modules m INNER JOIN pages_modules pm ON (pm.mid=m.mid) 
 INNER JOIN pages p ON(p.pid=pm.pid)
 WHERE p.weight=255 AND m.active='Y'  
 ...
 function get_logo() 
 {
  $sql = "SELECT content FROM `contents` WHERE active='Y' AND weight=".LOGO;
  $res = $this->mdb2->queryOne($sql);
  if (PEAR::isError($res)) {
    die($res->getMessage(). ' - line ' . __LINE__ . ': ' . $sql);
  }
 ...
 function get_latest_10($mid='') 
 {
  ...
  $sql = "SELECT cid, title, author, updated, mname
   FROM `contents`  ORDER BY updated DESC, cid DESC LIMIT 0, 10 ";
  $res = $this->mdb2->query($sql);
  if (PEAR::isError($res)) {
    die($res->getMessage(). ' - line ' . __LINE__ . ': ' . $sql);
  }
