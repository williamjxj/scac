...
class ListBase extends BaseClass
{
  ...
  function __construct()
  {
	...
	$this->html = '';
	$this->ary1 = array('users', 'levels');
	$this->username = isset($_SESSION['scac']['username'])?$_SESSION['scac']['username']:'scac';
	$this->userid = isset($_SESSION['scac']['userid'])?$_SESSION['scac']['userid']:2;
  ...
  function get_config($section_info) 
  {
	...
	if($this->self=='modules') {
		$query = "SELECT m.mid, p.name AS page, m.weight,
			m.name, m.description, m.active, m.createdby, m.created, m.updatedby, m.updated, m.url, m.url_flag
			FROM modules AS m
...
*/
  ...
  {
	$sql = "SELECT pid, name, description FROM pages p WHERE active='Y' ORDER BY name";
  ...
  function select_assigned_modules($pid)
  {
  	
	$sqlm = "SELECT m.mid, m.name, m.description, p.name FROM modules m INNER JOIN pages_modules pm INNER JOIN pages p ON (p.pid=pm.pid AND pm.mid=m.mid) WHERE pm.mid = m.mid  AND m.active='Y' AND p.pid=".$pid;
	if($pid) $sqlm .= "	AND pm.pid=".$pid;
	$sqlm .= " ORDER BY m.name ";
	return 	$this->get_select_options($sqlm, false);
  }  
  function select_available_modules($pid)
  {
	$sqlm = "SELECT m.mid, m.name, m.description, p.name FROM modules m 
		...
		WHERE m.active='Y' AND m.mid NOT IN (SELECT mid from pages_modules WHERE pid=".$pid.") ORDER BY m.name";
	return 	$this->get_select_options($sqlm, false);
  }

  function select_assigned_resources($cid)
  {
	$sqlr = "SELECT r.rid, file, author FROM resources r INNER JOIN contents_resources cr WHERE cr.rid = r.rid AND active='Y' AND cr.cid=".$cid." ORDER BY author ";
	return 	$this->get_select_options($sqlr, false);
  }  
  function select_available_resources($cid)
  {
	$sqlr = "SELECT rid, file, author FROM resources WHERE rid NOT IN (SELECT rid from contents_resources WHERE cid=".$cid.") AND active='Y' ORDER BY author";
	return 	$this->get_select_options($sqlr, false);
  }
