...
<?php /*%%SmartyHeaderCode:606704394510810176c20e1-87067385%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  ...
  'nocache_hash' => '606704394510810176c20e1-87067385',
  'function' => 
  array (
  ),
...
<?php $_tmp10=ob_get_clean();?><?php if ($_tmp10){?>
            <td align="right"> <?php if (preg_match("/(\.pdf|\.txt|\.xml|\.mid|\.sql|\.ppt)"."$"."/",(isset($_smarty_tpl->tpl_vars['v']->value) ? $_smarty_tpl->tpl_vars['v']->value : null))){?> <a href="<?php echo (isset($_smarty_tpl->tpl_vars['m']->value['Path']) ? $_smarty_tpl->tpl_vars['m']->value['Path'] : null);?>
...
<?php }?>
<form action="<?php echo (isset($_smarty_tpl->getVariable('config')->value['path']) ? $_smarty_tpl->getVariable('config')->value['path'] : null);?>
...
$('table#list_table tr.infoRow td a.email').bind('click', function(event) {
  ...
  success: function(data){
    if(data==0) {
	  alert('邮件成功发送, 请检查你的邮箱. Successfully sent the emails');
	}
	else {
	  alert('邮件发送失败. Sent emails Failure');
	}
  }
  });
  return false;
